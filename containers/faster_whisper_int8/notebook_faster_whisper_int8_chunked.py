"""
notebook_faster_whisper_int8_chunked.py — Benchmark: faster-whisper large-v3 int8, silence-chunked
=====================================================================================================
...
Why chunk at 3 minutes:
  - Whisper degrades on very long audio (attention sink, hallucination loops)
  - 3 min is well within the sweet spot for large-v3 accuracy
  - Silence-based splitting avoids cutting mid-word

Install (extra dep vs whole-audio version):
    pip install pydub

Run:
    python notebook_faster_whisper_int8_chunked.py
"""
...
RESULTS_PATH    = RESULTS_DIR / "results_faster_whisper_int8_chunked.csv"
...
SILENCE_THRESH_DB   = -40             # dBFS — louder = more aggressive splitting
...
def make_chunks(audio_path: Path) -> list[AudioSegment]:
    """
    Split audio on silence, then greedily merge pieces into chunks
    no longer than CHUNK_MAX_MS. Returns a list of AudioSegment chunks.
    """
    audio = AudioSegment.from_file(str(audio_path))

    # If the whole file fits in one chunk, skip splitting entirely
    if len(audio) <= CHUNK_MAX_MS:
        return [audio]

    # Split on silence into small pieces
    ...
    if not pieces:
        # No silence found — fall back to hard time slices
        log.warning("  No silence detected — falling back to hard 3-min slices")
        chunks = []
        for start in range(0, len(audio), CHUNK_MAX_MS):
            chunks.append(audio[start : start + CHUNK_MAX_MS])
        return chunks

    # Greedy merge: accumulate pieces until we'd exceed CHUNK_MAX_MS
