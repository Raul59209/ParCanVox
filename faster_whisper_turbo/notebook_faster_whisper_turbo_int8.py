"""
notebook_faster_whisper_turbo_int8.py — Benchmark: faster-whisper large-v3-turbo int8
=========================================================================================
Model:   faster-whisper large-v3-turbo, forced int8 quantization
Backend: CTranslate2

FIX (2026-06-23): faster-whisper's built-in model-size registry does NOT
include any "turbo" alias — WhisperModel("large-v3-turbo") fails with
"Invalid model size 'large-v3-turbo'". The registry only resolves a fixed
list of OpenAI's original size names (tiny, base, small, medium, large-v1/2/3,
distil-* variants). Turbo isn't in that list — it has to be pointed at an
actual Hugging Face repo ID of a pre-converted CTranslate2 turbo model.

Fixed by pointing MODEL_SIZE at Zoont/faster-whisper-large-v3-turbo-int8-ct2,
a community conversion of openai/whisper-large-v3-turbo that's already
quantized to int8 specifically for faster-whisper — this avoids a redundant
runtime requantization step too.

Results saved to results_faster_whisper_turbo_int8.csv

Run:
    python notebook_faster_whisper_turbo_int8.py
"""
...
# ── Config ────────────────────────────────────────────────────────────────────
# Real HF repo ID — NOT a bare size string, since faster-whisper has no
# built-in "turbo" alias in its model registry.
MODEL_SIZE      = "Zoont/faster-whisper-large-v3-turbo-int8-ct2"
MODEL_LABEL     = "large-v3-turbo"   # just for naming results/output, not passed to WhisperModel
LANGUAGE        = "fr"
DATASET_PATH    = Path("dataset/test_set_frozen.json")
AUDIO_BASE_DIR  = Path("audio")
RESULTS_DIR     = Path("results")
RESULTS_PATH    = RESULTS_DIR / "results_faster_whisper_turbo_int8.csv"
...
# ── Load model ────────────────────────────────────────────────────────────────
log.info(f"Loading {MODEL_SIZE} on {DEVICE} ({COMPUTE_TYPE})...")
try:
...
for idx, seg in enumerate(segments):
    ...
    records.append({
        "model":               f"faster-whisper-{MODEL_LABEL}-int8-whole",
        "model_repo":          MODEL_SIZE,
        "device":              DEVICE,
...
# ── Summary ───────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print(f"RESULTS — faster-whisper {MODEL_LABEL} int8 / whole audio ({DEVICE})")
print("=" * 60)
